...
This is a monorepo with two separate test environments:

**Backend (`server/`):**
- Test runner: Jest (`server/package.json`)
- Unit tests: `server/src/**/*.spec.ts`
- E2E tests: `server/test/**/*.e2e-spec.ts`
- Run with: `npm run test` / `npm run test:e2e` from `server/`
- NestJS testing utilities (`@nestjs/testing`, `Test.createTestingModule`)
- Mock WCL API responses (never call the real WCL API in tests — it has rate limits and requires credentials)
- Key things to test: WCL auth token caching, consumable buff ID mapping logic, resolver inputs/outputs

**Frontend (`client/`):**
- Test runner: Vitest (`client/vitest.config.ts`)
- Component tests: `client/src/**/*.spec.ts` or `client/src/**/*.test.ts`
- Testing Library: `@testing-library/vue`
- Run with: `npm run test` from `client/`
- Mock Apollo Client responses using `@vue/apollo-composable`'s mock provider or MSW
- Key things to test: consumable table rendering, fight selector, missing-consumable highlighting logic

**Do not** suggest Jest for the frontend or Vitest for the backend — each workspace has its own runner.

**No database, no Docker required** — the backend has no persistence layer; mock the WCL API client in backend tests.
