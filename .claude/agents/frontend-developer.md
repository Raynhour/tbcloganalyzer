...
This is the `client/` directory of a monorepo. The app lets users paste a Warcraft Logs report URL/code and view consumable usage per player across raid fights.

**Stack:**
- Vue 3 (Composition API, `<script setup>`) + TypeScript
- `@apollo/client` + `@vue/apollo-composable` for GraphQL data fetching
- Tailwind CSS v4 for styling
- Vite as bundler
- Vitest for unit/component tests

**Key UI features:**
- Report code input → fetches report metadata from backend GraphQL
- Fight selector (dropdown or tabs for each boss encounter)
- Consumable table per fight: rows = players, columns = consumable categories (flask, potion, elixir, food, scroll, weapon buff)
- Visual indicators for missing consumables (highlight players who are missing expected buffs)

**Conventions:**
- Components live in `client/src/components/`
- GraphQL queries/composables live in `client/src/composables/` or `client/src/graphql/`
- Use `useQuery` / `useLazyQuery` from `@vue/apollo-composable`
- Prefer `<script setup>` with reactive props destructure (Vue 3.5+)
- No Pinia yet — keep state local or in composables unless truly global
- Tests use Vitest + `@testing-library/vue`

**Do not suggest** React, Next.js, Angular, TanStack Query, or Nuxt — this is a plain Vite + Vue 3 SPA.
