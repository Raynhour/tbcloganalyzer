...
This is a monorepo web app that analyzes Warcraft Logs (WCL) combat reports for TBC consumable usage (potions, flasks, elixirs, food buffs, scrolls, weapon buffs) per player.

**Stack:**
- Backend: NestJS with `@nestjs/graphql` (code-first), `graphql-request` to call WCL API v2
- Frontend: Vue 3 + `@apollo/client` + `@vue/apollo-composable`
- WCL API: GraphQL API v2 at `https://www.warcraftlogs.com/api/v2/client` — authenticated via OAuth 2.0 client credentials (token endpoint: `https://www.warcraftlogs.com/oauth/token`)

**Key domain types to keep in mind:**
- `Report` — a WCL report (identified by a report code)
- `Fight` — a boss encounter within a report
- `Player` — a raid member (name, class, spec)
- `Consumable` — a buff applied to a player (type: potion/flask/elixir/food/scroll/weapon buff, name, itemId)
- `PlayerConsumables` — aggregated consumable usage per player per fight

**WCL API query patterns:**
- Use `reportData { report(code: $code) { ... } }` to fetch report metadata
- Use `events` or `table` queries with `type: "buffs"` to get buff/consumable data
- The WCL API has rate limits — avoid overfetching; use the `table` query over `events` where possible

**Schema conventions:**
- Code-first with NestJS decorators (`@ObjectType`, `@Field`, `@Resolver`, `@Query`, `@Args`)
- Keep WCL API types separate from the app's own GraphQL schema (WCL types live in `server/src/wcl/`)
- Apollo Client cache key: `Report:code` (not auto-id)
