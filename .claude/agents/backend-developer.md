...
## Project Context: WoW TBC Log Analyzer

This is the `server/` directory of a monorepo NestJS application that acts as a GraphQL API gateway between the Vue frontend and the Warcraft Logs (WCL) API v2.

**Stack:**
- NestJS with `@nestjs/graphql` (code-first, Apollo driver)
- `graphql-request` to call the WCL GraphQL API v2
- OAuth 2.0 client credentials flow for WCL authentication (token endpoint: `https://www.warcraftlogs.com/oauth/token`, API endpoint: `https://www.warcraftlogs.com/api/v2/client`)
- TypeScript throughout
- Jest for unit and e2e tests (in `server/test/`)

**Domain:**
The backend fetches WCL report data and transforms it into consumable-centric types. Key responsibilities:
- Authenticate with WCL via client credentials (cache the token, refresh on expiry)
- Query WCL for report metadata (fights, players/actors)
- Query WCL buff events/tables to detect consumable usage per player per fight
- Map WCL buff IDs/spell IDs to known TBC consumable categories (flask, potion, elixir, food, scroll, weapon buff)
- Expose this via a clean NestJS GraphQL schema to the frontend

**Module structure conventions:**
- `server/src/wcl/` — WCL API client, auth, raw query types
- `server/src/reports/` — Report resolver, service, DTOs
- `server/src/consumables/` — Consumable mapping logic, known buff ID lists

**Key constraints:**
- WCL API has rate limits — cache token and consider caching report data
- No database — all data is fetched from WCL API on demand
- Do not expose WCL client credentials to the frontend
